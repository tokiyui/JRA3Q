...
for i in range(len(maxid[0])):
  ...
  if (fig_z[0] > 0 and fig_z[0] < 1 and fig_z[1] > 0 and fig_z[1] < 1):
    ax.plot(wlon, wlat, marker='x', markersize=15, color="white", transform=latlon_proj)
    ax.text(wlon - 1, wlat + 1, 'H', size=30, color="white", transform=latlon_proj)
    val = dss['prmsl'].values[maxid[0][i]][maxid[1][i]]
    ival = int(val)
    ax.text(fig_z[0], fig_z[1] - 0.01, str(ival), size=30, color="white", transform=ax.transAxes, verticalalignment="top", horizontalalignment="center")
...
for i in range(len(minid[0])):
  ...
  if (fig_z[0] > 0 and fig_z[0] < 1 and fig_z[1] > 0 and fig_z[1] < 1):
    ax.plot(wlon, wlat, marker='x', markersize=15, color="white", transform=latlon_proj)
    ax.text(wlon - 1, wlat + 1, 'L', size=30, color="white", transform=latlon_proj)
