...
# wが5以下の場所のみフィルタリング
dss['prmsl'] = (["lat", "lon"], np.where(surf >= 8000, gaussian_filter(dss['prmsl'].values, sigma=3), dss['prmsl'].values) * units(elem_units[3]))
dss['prmsl'] = (["lat", "lon"], np.where(w <= 10, gaussian_filter(dss['prmsl'].values, sigma=3), dss['prmsl'].values) * units(elem_units[3]))
dss['prmsl'] = (["lat", "lon"], np.where(w <= 15, gaussian_filter(dss['prmsl'].values, sigma=3), dss['prmsl'].values) * units(elem_units[3]))
