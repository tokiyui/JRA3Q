...
ds4['tmp'] = (["level", "lat", "lon"], gaussian_filter(ds4['tmp'].values, sigma=4) * units(elem_units[1]))
...
z500 = ax.contour(ds4['lon'], ds4['lat'], ds4['hgt'].sel(level=500.0), levels=np.arange(4980, 6000, 60), colors='white', linewidths=3.0, linestyles='solid', transform=latlon_proj, alpha=0)
...
ax.clabel(ept850, ept850.levels, fontsize=11, inline=True, inline_spacing=1, fmt='%i', rightside_up=True)
