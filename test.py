...
w = gaussian_filter(np.sqrt(dss['10u'].values ** 2 + dss['10v'].values ** 2), sigma=3)
surf = gaussian_filter(surf, sigma=3)


# wが5以下の場所のみフィルタリング
dss['prmsl'] = (["lat", "lon"], np.where(surf >= 8000, gaussian_filter(dss['prmsl'].values, sigma=5), dss['prmsl'].values) * units(elem_units[3]))
dss['prmsl'] = (["lat", "lon"], np.where(w <= 5, gaussian_filter(dss['prmsl'].values, sigma=5), dss['prmsl'].values) * units(elem_units[3]))
dss['prmsl'] = (["lat", "lon"], np.where(w <= 10, gaussian_filter(dss['prmsl'].values, sigma=3), dss['prmsl'].values) * units(elem_units[3]))
